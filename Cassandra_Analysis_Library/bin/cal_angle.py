...
parser.add_argument('-o',action='store',help='Optional: Output file name',default='angle.xvg')

#DEFINE PARSER ARGS
args = parser.parse_args()
nspecies = len(args.m)
binwidth = args.deg
nslices = int(180.0/(args.deg))
xyzfile = args.f
...
angle = np.zeros(nslices+1,np.float)
#READ MCF FILES
...
ia = 0
print "Provide the index of the angle you would like to analyze:\n"
for i in range(nspecies):
	print "Species",str(i+1)+":\n"
	if molecule_info[i].nangles == 0:
		print "Species",str(i+1),"has no angles."

	for j in range(molecule_info[i].nangles):
		#temporarily declare atom indices and names 
		#(these will be overwritten once the user provides an index for the output)
		atom1_ndx = int(molecule_info[i].angles[j][0])-1
		atom1_type = molecule_info[i].atom_type[atom1_ndx]
		atom2_ndx = int(molecule_info[i].angles[j][1])-1
		atom2_type = molecule_info[i].atom_type[atom2_ndx]
		atom3_ndx = int(molecule_info[i].angles[j][2])-1
		atom3_type = molecule_info[i].atom_type[atom3_ndx]
		print "("+str(ia+1)+"):",atom1_type,atom2_type, atom3_type,\
			" (atom #: "+str(atom1_ndx+1)+"-"+str(atom2_ndx+1)+"-"+str(atom3_ndx+1)+")"
		ia += 1
	print "\n"
...
#need to determine species and angle index
species_ndx, angle_ndx = obtain_angle_indices(output_ndx)

#store atom indexes and names based on user input
atom1_ndx = int(molecule_info[species_ndx].angles[angle_ndx][0])-1
atom1_type = molecule_info[species_ndx].atom_type[atom1_ndx]
atom2_ndx = int(molecule_info[species_ndx].angles[angle_ndx][1])-1
atom2_type = molecule_info[species_ndx].atom_type[atom2_ndx]
atom3_ndx = int(molecule_info[species_ndx].angles[angle_ndx][2])-1
atom3_type = molecule_info[species_ndx].atom_type[atom3_ndx]
print "\n"
print "Angle",str(output_ndx+1),"(species",str(species_ndx+1)+") has been selected.\n"
print "Atoms",atom1_type+",",atom2_type+",","and",atom3_type,"will be analyzed.\n\n"
...
if (outfile == 'angle.xvg'):
	outfile = outfile[:-4]+'_'+atom1_type+'_'+atom2_type+'_'+atom3_type+'.xvg'

outfilew = open(outfile,'w')

for i in range(len(bins)):
	outfilew.write(outfmt%(bins[i],angle[i]))
	print soutfmt%bins[i],soutfmt%angle[i]
outfilew.close()
